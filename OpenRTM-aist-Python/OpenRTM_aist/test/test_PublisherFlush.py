...
class TestPublisherNew(unittest.TestCase):
	...
	def test_setBuffer(self):
		self.assertEqual(self._pf.setBuffer(None),OpenRTM_aist.DataPortStatus.PORT_ERROR)
		return

	def test_write(self):
		self.assertEqual(self._pf.write(123,0,0),OpenRTM_aist.DataPortStatus.PRECONDITION_NOT_MET)
		self.assertEqual(self._pf.setConsumer(Consumer()),OpenRTM_aist.DataPortStatus.PORT_OK)
		self.assertEqual(self._pf.write(123,0,0),123)
		return
