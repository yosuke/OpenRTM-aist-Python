...
class TestInPortPullConnector(unittest.TestCase):
	def setUp(self):
		...
		self._con = InPortPullConnector(OpenRTM_aist.ConnectorBase.Profile("name","id",[],OpenRTM_aist.Properties()),InPortConsumerMock())
	
	def test_read(self):
		data = []
		self.assertEqual(self._con.read(data),OpenRTM_aist.DataPortStatus.PORT_OK)
		return
	...
	def test_createBuffer(self):
		self._con.createBuffer(OpenRTM_aist.ConnectorBase.Profile("name","id",[],OpenRTM_aist.Properties()))
		return
